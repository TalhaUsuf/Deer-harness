"""
search_filesystem.py — Verbatim copy of `deerflow.sandbox.search`.

Why this is bundled with web-search
------------------------------------
Some agent harnesses combine **web** search/fetch with **filesystem**
search/grep so the LLM has a unified search affordance ("look online OR
in the local sandbox"). The two surfaces share semantics (regex query +
result list) and are often presented together. This module ships the
filesystem half — `find_glob_matches` and `find_grep_matches` — so a
caller can wire both kinds of search into one tool group without
reaching back into deerflow.sandbox.

Public functions
----------------
- `find_glob_matches(root, pattern, *, include_dirs, max_results)`
  → `(list[absolute_path], truncated_bool)`
- `find_grep_matches(root, pattern, *, glob_pattern, literal,
  case_sensitive, max_results, max_file_size, line_summary_length)`
  → `(list[GrepMatch], truncated_bool)` where each `GrepMatch` is a
  frozen dataclass with `path`, `line_number`, `line`.

Self-contained: stdlib only (os, re, fnmatch, pathlib, dataclasses).
