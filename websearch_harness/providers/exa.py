"""
exa.py — Exa.ai ``web_search`` + ``web_fetch`` tools.

Provider summary
----------------
- ``web_search_tool(query)`` → JSON list of ``{title, url, snippet}``;
  uses Exa's content highlights (``contents.highlights``) as snippets.
- ``web_fetch_tool(url)``    → ``# {title}\\n\\n{text[:4096]}`` via
  ``client.get_contents([url], text={"max_characters": 4096})``.

Configuration
-------------
- ``ToolConfig(name="web_search", api_key="$EXA_API_KEY", max_results=5,
  search_type="auto", contents_max_characters=1000)``.
- ``ToolConfig(name="web_fetch", api_key="$EXA_API_KEY")``.

External pip dep: ``exa-py>=1.0.0``.
