"""
tavily.py — Tavily AI ``web_search`` + ``web_fetch`` tools.
...
Both call into ``tavily.TavilyClient`` (the official ``tavily-python``
SDK).

Configuration
-------------
- ``ToolConfig(name="web_search", api_key="$TAVILY_API_KEY", max_results=5)``
  registered via ``set_tool_configs``.
- If no ToolConfig is registered, the SDK reads ``TAVILY_API_KEY`` from
  the environment by itself when ``api_key=None`` is passed.

External pip dep: ``tavily-python>=0.7.17``.
