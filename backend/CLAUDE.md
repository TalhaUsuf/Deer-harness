...
- `custom_instructions` - User-supplied thread-scoped system instructions; only honored on the first turn of a thread (see `CustomInstructionsMiddleware`)
...
3. **CustomInstructionsMiddleware** - On the first turn of a thread, reads `config.configurable.custom_instructions` (string) and prepends a `SystemMessage` wrapped in `<custom_instructions>...</custom_instructions>` to `state.messages`. The checkpointer persists the message so subsequent turns inherit it without re-passing. If `custom_instructions` is sent on a thread that already has prior turns, it is logged and silently ignored. Lead-agent only — not added to subagent runtime.
