"""
SandboxAuditMiddleware — audits bash tool calls for security violations
before they reach the sandbox executor.

Hook
----
`wrap_tool_call` / `awrap_tool_call` — runs before every tool call. Only
acts on tools named `bash` (or that pass a `command` string in tool args).

Three classifications, evaluated in two passes
----------------------------------------------
Pass 1 — whole-command scan against `_HIGH_RISK_PATTERNS`. If any matches,
the call is BLOCKED (returns an error ToolMessage explaining why).
Pass 2 — split on shell operators (`&&`, `||`, `;` — quote-aware) and
classify each sub-command independently against high-risk and
`_MEDIUM_RISK_PATTERNS`. Compound commands are blocked if ANY part is
high-risk; warned-but-executed if any part is medium-risk.

Audit log
---------
Every bash invocation (blocked or not) is emitted as a single structured
JSON line via `logger.info` so downstream observability stacks can ingest
it without parsing.

State
-----
Subclasses `AgentMiddleware[ThreadState]` to read `state.sandbox.sandbox_id`
for log attribution.

Internal deerflow imports (rewritten)
-------------------------------------
    deerflow.agents.thread_state.ThreadState
        → ._internal.thread_state.ThreadState
...
# Was: from deerflow.agents.thread_state import ThreadState
from ._internal.thread_state import ThreadState
