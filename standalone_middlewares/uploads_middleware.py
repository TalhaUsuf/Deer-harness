"""
UploadsMiddleware — injects metadata + outline of uploaded files into the
last HumanMessage so the LLM "sees" what the user attached.

Hook
----
`before_agent` — fires once per agent invocation. Resolves the thread's
uploads dir via `Paths`, then merges:
    new_files       — file dicts attached on the most recent HumanMessage
                      (read from `additional_kwargs.files`)
    historical_files — files already on disk in the thread's uploads dir

For each file, `extract_outline(file.with_suffix('.md'))` reads the sibling
Markdown sidecar (produced by your upload conversion pipeline) and pulls out
its heading structure so the LLM gets a navigable preview without reading
the whole document. Falls back to `_OUTLINE_PREVIEW_LINES=5` lines of the
file when no outline can be extracted.

The formatted block is wrapped in `<uploaded_files>...</uploaded_files>`
XML tags and PREPENDED to the last HumanMessage's content (in-place, but on
a copy of the message — original state is preserved).

Internal deerflow imports (rewritten)
-------------------------------------
    deerflow.config.paths.{Paths, get_paths}
        → ._internal.paths.{Paths, get_paths}
    deerflow.utils.file_conversion.extract_outline
        → ._internal.file_conversion.extract_outline
...
from ._internal.paths import Paths, get_paths
from ._internal.file_conversion import extract_outline
