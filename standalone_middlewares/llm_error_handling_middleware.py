"""
LLMErrorHandlingMiddleware — wraps every model call with retry/backoff and a
circuit-breaker pattern, classifying errors into retriable / quota / auth
buckets and converting them into recoverable assistant-facing responses.

Hook
----
`wrap_model_call` / `awrap_model_call` — runs around every model invocation.

Behaviour
---------
1. Up to 3 attempts on retriable failures (HTTP 408/409/425/429/500/502/503/504,
   network busy patterns). Honours `Retry-After` header if present.
2. Quota / auth errors return a friendly AIMessage immediately (no retry).
3. After `failure_threshold` consecutive failures across the process, the
   circuit opens — for `recovery_timeout_sec` further calls fail fast with
   "service temporarily unavailable" before any HTTP attempt is made.

Internal deerflow imports (rewritten)
-------------------------------------
    deerflow.config.get_app_config
        → ._internal.app_config.get_app_config
    Reads only `cfg.circuit_breaker.failure_threshold` and
    `cfg.circuit_breaker.recovery_timeout_sec`.
...
# Was: from deerflow.config import get_app_config
