"""
DeerFlowTodoListMiddleware — extends LangChain's `TodoListMiddleware` with
context-recovery and completion-enforcement behaviour.

Two extensions on top of the base class
---------------------------------------
1. `before_model`: if the previous turn called `write_todos` but that
   AIMessage has been scrolled out of the visible window (e.g. by
   summarisation), inject a HumanMessage (`todo_reminder`) reminding the
   model of the current todo list. Idempotent: if a `todo_reminder` is
   already present, skip.

2. `after_model`: if the model is about to exit (no tool_calls in latest
   AIMessage) but incomplete todos remain, inject a HumanMessage
   (`todo_completion_reminder`) and force a jump back to the model node.
   Capped at 2 reminders (`_MAX_COMPLETION_REMINDERS`) so the agent can
   eventually exit even if some todos legitimately cannot be completed.

State source
------------
Reads the current todo list from `state['todos']` (a list of `Todo` typed
dicts maintained by the base TodoListMiddleware via the `write_todos` tool).
