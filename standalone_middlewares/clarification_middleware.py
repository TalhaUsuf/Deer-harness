"""
ClarificationMiddleware — intercepts the `ask_clarification` tool call and
short-circuits the agent run so the user can answer.

Hook
----
`wrap_tool_call` / `awrap_tool_call` — fires for every tool call BEFORE it
executes. If the call's tool name is `ask_clarification`, the middleware
returns `Command(goto=END)` instead of letting the tool run, which terminates
the LangGraph run gracefully and surfaces the question to the user. For all
other tool calls it just delegates to `handler(request)` (no-op).

Behaviour highlights
--------------------
- `_stable_message_id` — derives a deterministic ToolMessage id from
  `tool_call_id + question`. This is necessary because LangGraph deduplicates
  messages by id; if we generated random UUIDs every time, repeated
  clarifications could produce duplicate state updates.
- `_is_chinese` — switches the formatted prompt between Chinese and English
  by counting CJK characters in the question.
- `_format_clarification_message` — adds visual icons per question type
  (text / multiple_choice / single_choice / boolean) and renders option
  labels for choice questions.

Internal deerflow imports: NONE — this file is fully standalone.
