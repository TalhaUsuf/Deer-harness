"""
ViewImageMiddleware — converts pending `view_image` tool results into a
multimodal HumanMessage so the LLM can actually "see" the image in the next
model call.

Hook
----
`before_model` / `abefore_model` — runs before every model invocation.

Trigger conditions
------------------
1. The previous AIMessage called the `view_image` tool.
2. ALL tool calls on that AIMessage have completed (matched ToolMessages
   exist for each tool_call_id).
3. `state.viewed_images` (populated by the `view_image` tool itself) has
   one or more entries.

When all three hold, the middleware appends a HumanMessage containing the
image as a multimodal content block:
...
Idempotency
-----------
After injection, the middleware emits an empty `viewed_images: {}` update,
which the `merge_viewed_images` reducer (defined in `_internal/thread_state.py`)
treats as an explicit clear — preventing re-injection on subsequent turns.
