"""
LoopDetectionMiddleware — detects repetitive tool-calling loops and
forces the agent to either warn or stop.

Two detection layers
--------------------
1. Hash-based: compute a stable hash of the multiset of tool calls in the
   most recent N AIMessages (default 20). If the same hash repeats >=3 times
   in a row, inject a warning HumanMessage. If it repeats >=5 times,
   hard-stop: strip tool_calls + raw `additional_kwargs.tool_calls` from
   the AIMessage and force a final text-only answer.

2. Frequency-based: count how many times each tool name has been called in
   the window. If a tool exceeds the per-tool warn threshold (default 30),
   inject a warning. If it exceeds the hard-limit (default 50), hard-stop.

State storage
-------------
Per-thread counters are stored in module-level `OrderedDict`s keyed by
thread_id, capped at `MAX_TRACKED_THREADS=100` (LRU eviction). All access
is guarded by a `threading.Lock` since LangGraph may dispatch concurrent
graph runs in the same process.

Hooks
-----
`after_model` / `aafter_model` — runs after every model response, inspects
the latest AIMessage's tool_calls, updates counters, and may rewrite the
message to remove tool_calls before it reaches the executor.
`reset` — clears state when a graph instance is recycled.
